  ...
  function displayFilteredActivities() {
    ...
    Object.entries(allActivities).forEach(([name, details]) => {
      ...
      if (currentDifficulty) {
        // Activities with no difficulty set are for all levels, so include them in every filtered view
  ...
  async function shareActivity(name, details) {
    ...
    if (navigator.share) {
      ...
      try {
        ...
        showMessage(`"${name}" link copied to clipboard!`, "success");
      } catch (error) {
        console.error("Clipboard copy failed:", error);
        showMessage("Could not copy link. Please share manually.", "error");
      }
    }
  }
